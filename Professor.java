...
public class Professor extends Person {
	...
	public Professor(String name, String title, int egn, int age, int height, int weight,  String otherJob, int salary) {
	...
	public static class ProfessorGenerator {
		...
		private static String[] jobs = {"Teacher", "Car Racer", "Office Manager"};
		public static Professor make() {
			...
			arrayIndex = ThreadLocalRandom.current().nextInt(0, jobs.length);
			String otherJob = jobs[arrayIndex];
			...
			int salary = ThreadLocalRandom.current().nextInt(0, 5000);
			
			return new Professor(name, title, egn, age, height, weight, otherJob, salary);
		}
	}
}
